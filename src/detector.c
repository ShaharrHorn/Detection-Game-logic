...
#ifdef OPENCV
#include "C:\opencv\opencv\build\include/opencv2/highgui/highgui_c.h"
#include "C:\opencv\opencv\build\include/opencv2/core/core_c.h"
//#include "opencv2/core/core.hpp"
#include "C:\opencv\opencv\build\include/opencv2/core/version.hpp"
#include "C:\opencv\opencv\build\include/opencv2/imgproc/imgproc_c.h"

#ifndef CV_VERSION_EPOCH
#include "C:\opencv\opencv\build\include/opencv2/videoio/videoio_c.h"
...
char labelstr[50] = "Click on the ";
char greatstr[50] = "Great";
char loststr[50] = "too bad";
char **global_names;
char* namess[12];
char* object_to_choose;
image **alphabet, original_image;;
                   ...
                   float hier_thresh, int dont_show, int ext_output, int save_labels)
{
	...
	global_names= get_labels_custom(name_list, &names_size);
    ...
    while(1){
		...
		original_image = load_image(input, 0, 0, net.c);
		//IplImage* img = cvLoadImage(input,1);
	
        int letterbox = 0;
        image sized = resize_image(original_image, net.w, net.h);
		//show_image(original_image, "Camera1");
		image im = copy_image(original_image);
		...
		detec = draw_detections_v3(im, dets, nboxes, thresh, names, alphabet, l.classes, ext_output);
		for (int i = 0; i < 3; i++)		// num_of_detections instead of 3
		{
			namess[i] = global_names[global_selected_detections[i].best_class];
		}
		time_t t;
		srand((unsigned)time(&t));
		int random = (rand() % (3));	// num_of_detections instead of 3
		char str[10];
		sprintf(str, "%d", score);
		char* object_name = namess[random];
		object_to_choose = object_name;
		strcat(labelstr, object_name);
		strcat(scorestr, str);
		float rgb[3] = { 100, 150, 200 };
		write_text_image(alphabet, labelstr, (im.h*.03), im,rgb,15, im.w - (im.w / 2 + im.w / 4));
		write_text_image(alphabet, scorestr, (im.h*.03), im, rgb, im.h-10, im.w - im.w/4);

	    sized = resize_image(im, net.w, net.h);
		show_image(im, "Camera");
...
char *get_object_to_choose() {
	time_t t;
	//if (num_of_detections == 1)			// if there is only 1 detection, need to think what to do.
	//	return;
	srand((unsigned)time(&t));
	int random = (rand() % (3));		// num_of_detections instead of 3
	while(strcmp(namess[random],object_to_choose)==0)
		 random = (rand() % (3)); // num_of_detections instead of 3
	return namess[random];
}
...
void run_detector(int argc, char **argv)
{
    ...
    if(0==strcmp(argv[2], "test")) test_detector(datacfg, cfg, weights, filename, thresh, hier_thresh, dont_show, ext_output, save_labels);
       ...
       // char *name_list = option_find_str(options, "names", "data/names.list");
		char *name_list = "C:\\Users\\shahar\\Desktop\\CPP_no_gpu\\build\\darknet\\x64\\data\\voc.names";
        char **names = get_labels(name_list);
        if(filename)
...
void on_mouse(int event, int x, int y, int flags, void* userdata)
{
	...
	image temp = copy_image(original_image);
	...
	if (event == CV_EVENT_LBUTTONDOWN)
	{
		...
		for (int i = 0; i < 3; i++)		// dets_num1
		{
			...
			strcpy(tempstr, global_names[global_selected_detections[i].best_class]);
			printf("\n");
			printf("detection positions X : %d, %d", x1, w1);
			printf("\n");
			printf("detection positions Y: %d, %d", y1, h1);
			printf("\n");
			if (x1 < x && x < w1 && h1 < y && y < y1 && (strcmp(tempstr, object_to_choose) == 0))
			{
				clicked = 1;
			}
		}
		...
		if (clicked == 1) {
			score += 10;
			printf("Clicked");
			write_text_image(alphabet, greatstr, (temp.h*.03), temp, rgb, 15, temp.w - (temp.w / 2 + temp.w / 4));
		}
		else {
			printf("Not Clicked");
			if (score > 0)
				score -= 5;
			printf("Clicked");
			write_text_image(alphabet, loststr, (temp.h*.03), temp, rgb, 15, temp.w - (temp.w / 2 + temp.w / 4));
		}

		char score_str[20] = "SCORE : ";
		sprintf(str, "%d", score);
		strcat(score_str, str);
		write_text_image(alphabet, score_str, (temp.h*.03), temp, rgb, temp.h - 10, temp.w - temp.w / 4);
	//	show_image(temp, "Camera1");

		new_object_to_choose = get_object_to_choose();
		object_to_choose = new_object_to_choose;
		temp = copy_image(original_image);
		strcpy(labelstr,"Click on the ");
		strcat(labelstr, new_object_to_choose);
		write_text_image(alphabet, labelstr, (temp.h*.03), temp, rgb, 15, temp.w - (temp.w / 2 + temp.w / 4));
		write_text_image(alphabet, score_str, (temp.h*.03), temp, rgb, temp.h - 10, temp.w - temp.w / 4);
